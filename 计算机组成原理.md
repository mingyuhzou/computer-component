...
**控制信号也是通过Pipeline register连续传递的**，因此对于ID/EX寄存器它的大小是**Readdata1(32)+Readdata2(32)+PC(32)+destination register address(5)+immediate(32)+EX(ALUOp2 Regdist1 ALUsrc1)+M(MemRead1 MemWrite1 Branch1)+EX(RegWrite1 MemtoReg1)=142**
...
这里的检测**以lw为视角**的，通过判断**MemRead**信号来判断是否是lw指令，只有lw指令会设置该信号，然后还要看**lw的目的寄存器是否与前一条指令的源寄存器冲突**。注意**LW指令是在EX阶段**，判断的**寄存器是rt**， 因为I型指令中**rt是目的寄存器**。。同时是在ID阶段判断的。
...
+ **不让IF/ID寄存器改变disable  write，这会使得ID阶段的指令重复(防止后面清空ID/EX寄存器时丢失下一条指令)**
...
缓存**将多个相邻的字节一起存储为一个缓存块**以此来利用**空间局部性**提高CPU的访问速度，并且缓存会**保存最近访问过的内存块并丢弃不常使用的**来利用时间局部性。
...
进程会使用的虚拟内存(virtual address)，每个进程有自己的空间地址，虚拟内存地址可以**大于**物理地址，**只有一部分的虚拟内存会映射到物理地址上**，进程的一部分内容储存在磁盘上，操作系统和硬件同时协作将数据从磁盘中移入/出。
...
缓存的大小(Cache Size)是指**所有block(data)大小的和而不包括tag和有效位**。
...
如下图所示比较**valid&(tag是否与地址的tag一致)**，**offset用于选择数据是在data字段的那一部分**，用多路选择器体现。

![{BE780362-A8EE-4B5B-83D8-C9061021BFAA}](./assets/{BE780362-A8EE-4B5B-83D8-C9061021BFAA}.png)



对于**16位**的机器，采用直接映射缓存的，每个块都只有**一个字节**
...
**Number of sets = cache size/(block size*ways)**，用way描述划分的个数，从0开始。
...
所有的**Cache line 都位于一个组中**，也就是数据可以存放到任何的cache block(data字段)中，此时不会再有任何冲突了。
...
Full associative Cache的缺点是：**因为数据会存放在任何地方，所以必须比较所有的tag，需要比较的次数太多了；成本高；增大了缓存的体积。**
